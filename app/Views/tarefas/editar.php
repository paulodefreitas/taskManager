...
<main class="flex-shrink-0">
  <div class="container">
    ...
    <div class="row">
      <div class="col-sm">
        <h3 class="text-center text-secondary">Atualizar tarefa</h3>
      </div>
    </div>
    ...
    <form name="cadastrar" method="POST" action="<?= URL ?>/tarefas/editar/<?= $dados['id'] ?>">
      ...
      <div class="row mb-3">
        ...
        <select class="form-select <?= $dados['prioridade_erro'] ? 'is-invalid' : '' ?>" id="prioridade" name="prioridade">
            <option value="" disabled selected>Selecione a prioridade da tarefa</option>
        ...
        <div class="col-sm">
          ...
          <select class="form-select <?= $dados['estado_erro'] ? 'is-invalid' : '' ?>" id="estado" name="estado">
            <option value="" disabled selected>Selecione o estado da tarefa</option>
            <option value="Executada" <?= $dados['estado'] == 'Executada' ? 'selected' : '' ?>>Executada</option>
      ...
      <div class="row justify-content-end">
        <div class="col-sm-auto">
          <input type="image" class="btn btn-secondary" src="<?= URL ?>/img/saveWhite.svg" title="Atualizar Tarefa"/>
        </div>
      </div>
    </form>
  </div>
